// Claude Plugin Manager - Table-based UI with Skills Support (VS Code WebView Edition)

// VS Code WebView API adapter
const IS_VSCODE = typeof acquireVsCodeApi !== 'undefined';
const API_BASE = IS_VSCODE ? '' : 'http://localhost:3456';
...
// Unified API request function (VS Code or HTTP)
async function apiRequest(endpoint, options = {}) {
    if (IS_VSCODE) {
        // VS Code WebView mode - use postMessage
        return new Promise((resolve, reject) => {
            const messageId = Date.now() + Math.random();

            // Extract type from endpoint
            let type = endpoint.replace('/api/', '').replace('/', '_');
            // /api/plugins -> getPlugins
            // /api/plugins/{id}/toggle -> togglePlugin
            // /api/skills -> getSkills
            if (endpoint.includes('/toggle')) {
                type = 'togglePlugin';
            } else if (endpoint.includes('/enable')) {
                type = 'enablePlugin';
            } else if (endpoint.includes('/disable')) {
                type = 'disablePlugin';
            } else if (endpoint.startsWith('/api/plugins')) {
                type = 'getPlugins';
            } else if (endpoint.startsWith('/api/skills')) {
                type = 'getSkills';
            } else if (endpoint.startsWith('/api/commands')) {
                type = 'getCommands';
...
async function loadAllStats() {
    try {
        // Load skills count
        const skillsRes = await apiRequest('/api/skills');
        if (skillsRes.ok) {
        ...
        // Load commands count
        const commandsRes = await apiRequest('/api/commands');
        if (commandsRes.ok) {
        ...
        // Load agents count
        const agentsRes = await apiRequest('/api/agents');
        if (agentsRes.ok) {
...
async function viewSkill(skillId) {
    try {
        const response = await apiRequest(`/api/skills/${encodeURIComponent(skillId)}`);
...
async function togglePlugin(pluginId) {
    ...
    try {
        const response = await apiRequest(`/api/plugins/${pluginId}/toggle`, {
            method: 'POST'
        });
...
async function enableCategory(marketplace) {
    ...
    for (const plugin of categoryPlugins) {
        if (!plugin.enabled) {
            try {
                await apiRequest(`/api/plugins/${plugin.id}/enable`, { method: 'POST' });
                plugin.enabled = true;
            } catch (error) {
...
async function disableCategory(marketplace) {
    ...
    for (const plugin of categoryPlugins) {
        if (plugin.enabled) {
            try {
                await apiRequest(`/api/plugins/${plugin.id}/disable`, { method: 'POST' });
...
async function enableAllPlugins() {
    showConfirmModal(
        ...
        async () => {
            try {
                const response = await apiRequest(`/api/plugins/enable-all`, {
                    method: 'POST'
                });
...
async function disableAllPlugins() {
    showConfirmModal(
        ...
        async () => {
            try {
                const response = await apiRequest(`/api/plugins/disable-all`, {
                    method: 'POST'
                });
    ...
    showConfirmModal(
        ...
        async () => {
            try {
                ...
                const response = await apiRequest(`/api/plugins/${pluginId}/update`, {
                    method: 'POST'
                });
...
async function uninstallPlugin(pluginId) {
    ...
    showConfirmModal(
        ...
        async () => {
            try {
                const response = await apiRequest(`/api/plugins/${pluginId}`, {
                    method: 'DELETE'
                });
    ...
    showConfirmModal(
        ...
        async () => {
            try {
                ...
                const response = await apiRequest(`/api/plugins/update-all`, {
                    method: 'POST'
                });
...
async function saveConfig() {
    try {
        const response = await apiRequest(`/api/plugins/save`, {
            method: 'POST'
        });
...
async function viewCommand(commandId) {
    try {
        const response = await apiRequest(`/api/commands/${encodeURIComponent(commandId)}`);
...
async function viewAgent(agentId) {
    try {
        const response = await apiRequest(`/api/agents/${encodeURIComponent(agentId)}`);
...
function showEditModal(type, item) {
    ...
    document.getElementById('modalSaveBtn').addEventListener('click', async () => {
        ...
        try {
            const response = await apiRequest(`/api/${type}s/${encodeURIComponent(name)}`, {
                method: 'POST',
...
async function loadMarketplaceExtensions() {
    try {
        const response = await apiRequest('/api/marketplace/extensions');
...
async function showExtensionDetail(ext) {
    ...
    // Fetch full details
    try {
        const response = await apiRequest(`/api/marketplace/extensions/${encodeURIComponent(ext.id)}`);
...
async function installExtension() {
    ...
    try {
        ...
        const response = await apiRequest(`/api/marketplace/extensions/${encodeURIComponent(selectedExtension.id)}`);
        ...
        // Install
        const installResponse = await apiRequest(`/api/marketplace/install`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
...
async function showMarketplaceSettings() {
    ...
    let currentPAT = '';
    try {
        const response = await apiRequest('/api/marketplace/settings');
        if (response.ok) {
            const data = await response.json();
            currentPAT = data.pat || '';
        }
...
async function loadSecurityResults() {
    try {
        const response = await apiRequest('/api/security/history');
...
async function startScan() {
    ...
    try {
        ...
        // Start scan
        const response = await apiRequest(`/api/security/scan`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function pollScanStatus(scanId) {
    ...
    scanPollInterval = setInterval(async () => {
        try {
            ...
            const response = await apiRequest(`/api/security/scan/${scanId}`);
            if (!response.ok) {
                clearInterval(scanPollInterval);
...
function createHistoryCard(record) {
    ...
    historyCard.onclick = async () => {
        try {
            const response = await apiRequest(`/api/security/history/${record.id}`);
...
async function deleteHistoryRecord(recordId) {
    try {
        ...
        const response = await apiRequest(`/api/security/history/${recordId}`, {
            method: 'DELETE'
        });
...
async function checkClaudeCliAvailability() {
    try {
        const response = await apiRequest('/api/security/cli-status');
